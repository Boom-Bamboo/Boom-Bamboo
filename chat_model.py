...
class ChatModel(nn.Module, GenerationMixin):
    ...
    def chat(self, tokenizer, query: str, history: List[Tuple[str, str]] = None, max_length: int = 2048, num_beams=1,
             do_sample=False, top_p=0.75, temperature=0.1, logits_processor=None, **kwargs):
        if history is None:
            history = []
    ...
    @torch.no_grad()
    def batch_generate(self, tokenizer, queries, max_length: int = 2048, num_beams=1, do_sample=False, top_p=0.75, temperature=0.1, **kwargs):
        gen_kwargs = {"max_length": max_length, "num_beams": num_beams, "do_sample": do_sample, "top_p": top_p, "temperature": temperature, **kwargs}
